...
let cc = ['ad','ae','af','ag','ai','al','am','ao','aq','ar','as','at','au','aw','ax','az','ba','bb','bd','be','bf','bg','bh','bi','bj','bl','bm','bn','bo','bq','br','bs','bt','bv','bw','by','bz','ca','cc','cd']
let howmany = 3; // 3
let howmanyInGame = 10;
...
function setup() {
  ...
  selectedCountriesGame = selectRandom(cc, howmanyInGame);
...
function getCapital(countryCode, language) {
  ...
  const errorMessage = {
    "fi": "Pääkaupunkia ei löytynyt annetulle maakoodille",
    "sv": "Huvudstaden hittades inte för angiven landskod",
    "en": "Capital not found for the specified country code",
    "es": "Capital no encontrada para el código de país"
  }[language];

  return capital || errorMessage;
}

function getCountry(countryCode, language) {
  const country = countriesData[countryCode]?.[language]?.country;
  const errorMessage = {
    "fi": "ei löytynyt annetulle maakoodille",
    "sv": "hittades inte för angiven landskod",
    "en": "not found for the specified country code",
    "es": "no encontrada para el código de país"
  }[language];
...
function draw() {
  ...
  if (x > width / 2 | x < -width / 2 | y > height / 2 | y < -height / 2) {
    selected_flag = (selected_flag + 1) % cc.length;
    resetDrag();
  }


  image(images[selected_flag], width / 2 - images[selected_flag].width / 2 + x, height / 2 - images[selected_flag].height / 2 + y);
...
function touchMoved() {
  ...
  vx = vx + (mouseX - pmouseX) * weightslide;
  vy = vy + (mouseY - pmouseY) * weightslide;
